...
//		}
	...
	public static Boolean settingsSet() {
		// check if the settings for ftp and mail are set, if not return false
		Configuration conf = HelloWorldBasic.userAccount.getConfiguration();
		if ((conf.getFetchMailConncetions().size() == 0)
				|| (conf.getFileTransferConncetions().size() == 0)
				|| (conf.getSendMailConncetions().size() == 0)) {
			return false;
		} else {
			return true;
		}
	...
	public static Bitmap getAndSetBitmapFromNet(String urlPath) {
		...
		if (urlPath != null) {
			try {
				...
				// Test-URLs
				// urlPath = "http://img3.sidereel.com/_icon/24a0.jpg";
				// urlPath
				// ="http://www.tutorialized.com/upload/(2006.06.25-18:52:33)monkey50x50.jpg";
				// urlPath ="http://www.gimpusers.de/dl/affe_ausgangsbild.jpg";
				// urlPath
				// ="http://www.xtreme-simpsons.de/pics/whatiswhat/affe.gif";
				// urlPath ="http://roflnet.de/archiv/inhalt/kontakt/Affe.png";
				// urlPath
				// ="http://devcentral.f5.com/Portals/0/images/About/white_monkey.bmp";
				// urlPath ="http://www.adpulp.com/untitled.bmp";

				/*
				 * url = new URL(urlPath); URLConnection conn =
				 * url.openConnection(); Log.i(I,"open Connection successful");
				 * conn.connect(); Log.i(I,"connect successful"); InputStream is
				 * = conn.getInputStream();
				 * Log.i(I,"get InputStream successful"); BufferedInputStream
				 * bis = new BufferedInputStream(is,512);
				 * Log.i(I,"convert InputStream to BufferedInputstream successful"
				 * ); Bitmap bm = BitmapFactory.decodeStream(bis);
				 * Log.i(I,"create Bitmap successful"); bis.close(); is.close();
				 */
				// setProgressBarIndeterminateVisibility(true);
				BufferedInputStream bis = new BufferedInputStream(new URL(
						urlPath).openStream(), 1024);
	...
	private static Bitmap processBitmap(Bitmap bm) {
		...
		bm = Bitmap.createScaledBitmap(bm, (int) newWidth, (int) newHeight,
				true);
	...
	public void getHCardTest() {
		StructuredProfileAttribute<String, Attribute> hcard = HelloWorldBasic.userAccount
				.getPrivateProfile().getHCard();
	...
	public static String[] getImageUrls() {
		...
		String[] urls = new String[HelloWorldBasic.userAccount.getContacts()
				.size()];
		...
		for (Contact c : HelloWorldBasic.userAccount.getContacts()) {
			...
			if (friendProfile != null) {
				try {
					imageUrl = friendProfile.getHCard().getPhoto().get(0)
							.getValue();
				} catch (Exception e) {
